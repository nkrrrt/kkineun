...
/**
 * 은행 입출금 내역을 어두운 화면에서 읽은 것. 토스와 생김새가 아주 다르다.
 *
 *   - 날짜가 '9.7' 이고, 그날 첫 줄에만 붙는다 ('9월 7일' 이 아니다)
 *   - 그 날짜가 이름·금액과 같은 줄에 있다
 *   - 시각과 잔액이 다음 줄에 함께 온다
 *
 * 깨진 모양은 실제로 읽힌 그대로 두고, 가게 이름과 금액만 지어낸 것으로
 * 바꿨다. '9.1' 이 '01' 로 읽힌 것까지 그대로다.
 */
const 은행화면 = `&  UERIKSZ (RIE... (<) ©
전체 v                                Q
9.7  현대카드(주)   -1,205 300
18:31           480215%
ost 302044    -12,400
18:27          1,240,500%
동양생09-041     -55,200%
...
롯데카드(주)     -180 000%
08:33          1 459200
110022334455 -500,000&
08:16          1 639200%
9.3 __ 네이버페이결제    -3,100원
11:53          2.139.200
01   토뱅어느사람    -2,000,000원
13:19           2139200원`;

test('은행 입출금 화면의 여덟 건을 모두 읽는다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  const rows = win.parseBankText(은행화면, 2026);
  const 뽑은것 = JSON.parse(JSON.stringify(rows.map((r) => [r.date, r.amount])));
  assert.deepEqual(뽑은것, [
    ['2026-09-07', 1205300],
    ['2026-09-07', 12400],
    ...
    ['2026-09-03', 3100],
    ['2026-09-01', 2000000],
  ], '뽑은 것: ' + JSON.stringify(뽑은것));
});

test("'9.7' 처럼 달과 날만 있는 날짜를 읽는다", { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // 날짜가 이름·금액과 같은 줄에 있으므로, 날짜만 떼고 나머지는 살려야 한다
  const rows = win.parseBankText('9.7  어느가게   -1,200원\n18:31   50,000원', 2026);
  assert.equal(rows.length, 1);
  assert.equal(rows[0].date, '2026-09-07');
  assert.equal(rows[0].amount, 1200);
  assert.equal(rows[0].raw, '어느가게');
});

test('마침표로 나뉜 금액을 날짜로 오해하지 않는다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // '3.022.697' 은 3월 2일이 아니라 쉼표가 마침표로 읽힌 금액이다.
  // 날짜라면 뒤에 숫자나 구분점이 더 붙지 않는다.
  const rows = win.parseBankText('9.7  어느가게  -1,200원\n3.022.697\n18:31', 2026);
  assert.equal(rows.length, 1);
  assert.equal(rows[0].date, '2026-09-07');
});

test('이름에 붙은 번호를 금액으로 읽지 않는다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // '동양생09-041' 의 '-041' 을 금액으로 보면 55,200원짜리가 41원이 된다.
  // 진짜 금액의 부호는 이름과 떨어져 있다.
  // jsdom 안에서 만든 배열은 바깥 배열과 종류가 달라 그대로는 못 비교한다
  const 돈 = JSON.parse(JSON.stringify(win.moneyIn('동양생09-041     -55,200%')));
  assert.deepEqual(돈.map((m) => m.value), [55200], JSON.stringify(돈));
});

test('날짜 앞자리를 놓쳐도 최신순을 보고 되살린다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // '9.1' 이 '01' 로 읽힌다. 은행 목록은 늘 최신순이라, 앞 날짜보다 앞선
  // 날이면 같은 달의 날로 본다.
  const rows = win.parseBankText(
    '9.7  어느가게  -1,200원\n18:31\n01  다른가게  -3,400원\n13:19', 2026);
  assert.equal(rows.length, 2);
  assert.equal(rows[1].date, '2026-09-01');
  assert.equal(rows[1].amount, 3400);
});

test('이름 앞의 아이콘 찌꺼기를 날짜로 오해하지 않는다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // '8 ㅎ주자유시장상인회' 의 '8' 은 아이콘이 글자로 읽힌 것이지 8일이 아니다.
  // 앞자리가 0 인 것만 놓친 날짜로 본다.
  const rows = win.parseBankText(토스화면, 2026);
  assert.equal(rows[0].date, '2026-08-30', '아이콘을 날짜로 봤습니다');
});

test('계좌번호로 보낸 이체는 번호를 이름으로 쓴다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // 숫자만 있는 줄은 대개 주문번호라 이름에서 뺀다. 다만 그것뿐이면
  // 그거라도 써야 한다 — 계좌번호로 보낸 이체가 그렇다.
  const rows = win.parseBankText('9.7 110022334455 -500,000원\n08:16', 2026);
  assert.equal(rows.length, 1);
  assert.equal(rows[0].raw, '110022334455');
});


/**
 * 간편결제 결제내역을 밝은 화면에서 읽은 것. 또 다른 생김새다.
 *
 *   - 달은 화면 맨 위에만 있고, 아래는 '8일 화요일' 처럼 날만 적힌다
 *   - 금액이 먼저 오고 상호가 그 아랫줄에 온다
 *   - 상호 뒤에 막대로 결제수단이 붙는다
 *
 * 깨진 모양은 읽힌 그대로 두고 상호와 금액만 지어낸 것으로 바꿨다.
 * '7,000' 이 '/,000' 으로, '9월' 머리글이 '« oy' 로 뭉개진 것까지 그대로다.
 */
const 결제내역화면 = `<       « oy        at
...
5일 토요일
-15,600원
(주)하얀어느제과 | 네이버페이 우리카드 체크
-10,500원
어느커피 | 네이버페이 우리카드 체크
-/,000원
어느행궁 어느숲 | 네이버페이 우리카드 체크
-12,500원
어느플레이 | 네이버페이 우리카드 체크
-2,900원
어느스토어6 store | 네이버페이 우리카드 체크
-3.800원
GS25 +H ㅣ네이버페이 우리카드 체크
-20,000원
AKPLAZA 어느점 |네이버페이 우리카드 체크
...
test('간편결제 결제내역 아홉 건을 모두 읽는다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // 달은 화면에서 못 읽었으므로 지금 보고 있는 달(9월)을 쓴다
  const rows = win.parseBankText(결제내역화면, 2026, 9);
  const 뽑은것 = JSON.parse(JSON.stringify(rows.map((r) => [r.date, r.amount])));
  assert.deepEqual(뽑은것, [
    ['2026-09-08', 32400],
    ['2026-09-05', 15600],
...
test('달이 화면 맨 위에만 있어도 날짜를 맞춘다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // '9월' 머리글이 읽히면 그것을 쓴다
  const rows = win.parseBankText('9월\n8일 화요일\n-1,200원\n어느가게', 2026, 3);
  assert.equal(rows[0].date, '2026-09-08');

  // 못 읽었으면 지금 보고 있는 달을 쓴다
  const rows2 = win.parseBankText('8일 화요일\n-1,200원\n어느가게', 2026, 5);
  assert.equal(rows2[0].date, '2026-05-08');
});

test('금액과 이름이 줄을 나눠 서도 건을 가른다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // 새 줄에 이름이 없어도, 읽던 건이 이미 금액과 이름을 갖췄으면 새 건이다
  const rows = win.parseBankText(
    '8일 화요일\n-1,200원\n어느가게\n-3,400원\n다른가게', 2026, 9);
  assert.equal(rows.length, 2, '두 건이 하나로 붙었습니다: ' + JSON.stringify(rows));
  assert.deepEqual(JSON.parse(JSON.stringify(rows.map((r) => r.amount))), [1200, 3400]);
});

test('쉼표 앞에 선 빗금은 7 로 읽는다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // '7,200원' 이 '/,200원' 으로 읽힌다. 안 고치면 200원이 된다.
  assert.deepEqual(
    JSON.parse(JSON.stringify(win.moneyIn('어느커피  -/,200원'))).map((m) => m.value),
    [7200]);

  // 숫자가 뒤따르는 빗금은 그냥 찌꺼기다. 7 을 덧붙이면 안 된다.
  assert.deepEqual(
    JSON.parse(JSON.stringify(win.moneyIn('어느영  -/72,580원'))).map((m) => m.value),
    [72580]);
});

test('상호 뒤에 막대로 붙은 결제수단을 뗀다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  assert.equal(win.tidyName('어느마켓 | 네이버페이 우리카드 체크'), '어느마켓');
  assert.equal(win.tidyName('AKPLAZA 어느점 |네이버페이 우리카드 체크'), 'AKPLAZA 어느점');

  // 막대만 보고 자르면 안 된다. 'T' 가 낱자 'ㅣ' 로 읽힌 것을 자르면
  // '카카오T바이크' 가 '카카오' 가 되어 버린다.
  assert.equal(win.tidyName('카카오 ㅣ 바이크'), '카카오T바이크');
});

test('괄호에 되풀이된 상호를 지운다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // 은행 내역은 '씨유(CU)권선' 처럼 한글과 영문을 나란히 적는다
  assert.equal(win.tidyName('씨유(0ㅇ4)권선'), 'CU 권선');

  // 같은 상호일 때만 지운다. 지점 이름은 그대로 둔다.
  assert.equal(win.tidyName('CU (본점)'), 'CU (본점)');
});

/**
 * 밝은 화면에서 읽은 통장 내역. 날짜 칸이 '9.2' → '22', '9.1' → '91' 로
 * 뭉개진 것까지 읽힌 그대로 두었다. 맨 위 한 건은 날짜가 화면 밖이라
 * 가져오지 않아야 맞다.
 */
const 통장화면 = `& .、 어느50060주거래 통장
AKPLAZA         -20,000원
13:12             361,639원

9.4 。_ 씨유(0ㅇ4)권선         -6,700원
18:32             381,639
어느백화점          -33,700원
11:52             388 339%
어느커피판교알         -3.800원
...
91 _ 어느커피판교알        -10,800원
12:09             5645999

채우기        보내기`;

test('통장 화면 열 건의 날짜와 금액이 모두 맞는다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  const rows = win.parseBankText(통장화면, 2026, 9);
  const 뽑은것 = JSON.parse(JSON.stringify(rows.map((r) => [r.date, r.amount])));
  assert.deepEqual(뽑은것, [
    ['2026-09-04', 6700],
    ['2026-09-04', 33700],
...
test('날짜 칸이 두 자리로 뭉개져도 되살린다', { skip }, async () => {
  const { win } = openApp();
  await wait(200);

  // '9.2' 가 '22' 로, '9.1' 이 '91' 로 읽힌다. 두 자리를 그대로 읽어 날이
  // 되면 그것을, 안 되면 끝자리를 쓴다. 목록이 최신순인 것이 잣대다.
  const rows = win.parseBankText(
    '9.4  어느가게  -1,000원\n18:32\n22 다른가게 -2,000원\n17:10\n91 또다른가게 -3,000원\n12:09',
    2026, 9);
  const 날짜 = JSON.parse(JSON.stringify(rows.map((r) => r.date)));
